...
def main(algorithm="mcts_vs_uniform", MCTS_iterations=100, debug_flag=0, num_game_play=1):
    Debug.debug = debug_flag
    winner_lst = []
    lst = [0, 0]
    for _ in tqdm.tqdm(range(num_game_play)):
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        main(debug_flag=1, num_game_play=int(sys.argv[1]))
    elif len(sys.argv) == 3 and sys.argv[1] == "random_vs_uniform":
        main(algorithm=sys.argv[1], debug_flag=1, num_game_play=int(sys.argv[2]))
    elif len(sys.argv) == 4 and (sys.argv[1] == "mcts_vs_uniform" or sys.argv[1] == "mcts_vs_random" ):
        main(algorithm=sys.argv[1], MCTS_iterations=int(sys.argv[2]), debug_flag=1, num_game_play=int(sys.argv[3]))
    elif len(sys.argv) == 5 and sys.argv[1] == "mcts_vs_mcts":
