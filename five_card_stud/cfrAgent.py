...
class CFRRootChanceGameState(CFRGameStateBase):

    def __init__(self, actions):
        super().__init__(parent = None, to_move = CHANCE, actions = actions)
        self.children = { #
            cards: CFRPlayerMoveGameState(
                self, A, [],  cards, [BET, CHECK]
            ) for cards in self.actions # chance node [*][*] [*]
        }
...
class CFRPlayerMoveGameState(CFRRootChanceGameState):
